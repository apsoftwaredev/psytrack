...
@synthesize useSCObjectsSelectionSection;
...
- (void)generateSections
{
    ...
    /* Note: sectionGenerated has been deprecated */
...
- (void)itemModified:(NSObject *)item inSection:(SCArrayOfItemsSection *)section
{
	...
	else
	{
        ...
        NSArray *sectionHeaderTitles = [self getSectionHeaderTitles];
        if( section.items.count || (sectionHeaderTitles && [sectionHeaderTitles indexOfObject:section.headerTitle]!=NSNotFound) )
        {
...
- (void)searchBar:(UISearchBar *)sbar textDidChange:(NSString *)searchText
{
	...
	if([sbar.text length])
	{
		NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF contains[cd] %@", sbar.text];
...
{
	return SC_Autorelease([[[self class] alloc] initWithTableView:(UITableView *) _modeledTableView withViewController:(UIViewController *)_viewController withEntityClassDefinition:(SCClassDefinition *)classDefinition usingPredicate:(NSPredicate *)predicate]);
}
#endif
...
{
    useSCObjectsSelectionSection=_useSCSelectionSection;
	return [self initWithTableView:_modeledTableView withViewController:_viewController
         withEntityClassDefinition:classDefinition usingPredicate:nil];
}
...
useSCObjectsSelectionSection=_useSCSelectionSection;
return  [  self initWithTableView:(UITableView *)_modeledTableView
withViewController:(UIViewController *)_viewController
withEntityClassDefinition:(SCClassDefinition *)classDefinition
                   usingPredicate:(NSPredicate *)predicate];
...
- (SCArrayOfItemsSection *)createSectionWithHeaderTitle:(NSString *)title
{
    ...
    if (self.tag==0 && useSCObjectsSelectionSection) 
        return [SCObjectSelectionSection sectionWithHeaderTitle:title withItems:[NSMutableArray array] 
