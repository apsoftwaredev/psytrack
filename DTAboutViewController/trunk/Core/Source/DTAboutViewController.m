...
- (BOOL) openExternalMailWithAddress:(NSString *)address subject:(NSString *)subject body:(NSString *)body;
...
- (void) dealloc
{
    ...
    [customViews release];
    [_layout release];
    [_appScrollerView release];
    [commandsForShowingActionSheet release];
...
- (UIView *) viewForDictionary:(NSDictionary *)dictionary atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([type isEqualToString:@"Image"])
    {
        ...
        if ([imageURL isFileURL])
        {
            ...
            contentView = [[[UIImageView alloc] initWithImage:image] autorelease];
    ...
    if ([type isEqualToString:@"Button"])
    {
        ...
        if (style)
        {
            button = [[[DTButton alloc] initWithFrame:CGRectMake(0, 0, maxWidth, 45.0)] autorelease];
    ...
    {
        DTFAQTable *faq = [[[DTFAQTable alloc] initWithFrame:CGRectMake(0, 0, maxWidth, 300.0)] autorelease];
...
- (UITableViewCell *) tableView:(UITableView *)tableView configureCellForDict:(NSDictionary *)rowDict atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil)
    {
        cell = [[[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:customKey] autorelease];
...
- (void) performAboutAction:(NSString *)action withObject:(id)object
{
    ...
    if ([command isEqualToString:@"actionsheet"])
    {
        ...
        [actionSheet addButtonWithTitle:NSLocalizedStringFromTableInBundle(@"Cancel", @"DTAboutViewController", resourceBundle, @"Cancel Button")];
    ...
    else if ([command isEqualToString:@"openmail"])
    {
        ...
        {
            if (![self openExternalMailWithAddress:address subject:subject body:body])
            {
                NSString *resourceBundlePath = [[NSBundle mainBundle] pathForResource:@"DTAboutViewController" ofType:@"bundle"];
                NSBundle *resourceBundle = [NSBundle bundleWithPath:resourceBundlePath];

                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTableInBundle(@"NO_MAIL_TITLE", @"DTAboutViewController", resourceBundle, @"Message to show when no email is configured")
                                                                message:NSLocalizedStringFromTableInBundle(@"NO_MAIL_MSG", @"DTAboutViewController", resourceBundle, @"Message to show when no email is configured") delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
                [alert show];
                [alert release];
            }
        }
